...
io.on('connection', function (socket) {
    ...
    socket.on('gotit', function (player) {
        console.log('[INFO] Oyuncu ' + player.name + ' bağlandı!');

        if (util.findIndex(users, player.id) > -1) {
            console.log('[INFO] Oyuncu bağlandı, bağlanma sorunlarından dolayı atıldı.');
            socket.disconnect();
        } else if (!util.validNick(player.name)) {
            socket.emit('kick', 'Geçersiz kullanıcıadı.');
            socket.disconnect();
        } else {
            console.log('[INFO] Oyuncu ' + player.name + ' bağlandı!');
    ...
    socket.on('respawn', function () {
        ...
        console.log('[INFO] Kullanıcı ' + currentPlayer.name + ' doğdu!');
    });

    socket.on('disconnect', function () {
        if (util.findIndex(users, currentPlayer.id) > -1) users.splice(util.findIndex(users, currentPlayer.id), 1);
        console.log('[INFO] Kullanıcı ' + currentPlayer.name + ' bağlantısı kesildi!');
        ...
        if (currentPlayer.admin) {
            ...
            for (var e = 0; e < users.length; e++) {
                if (users[e].name === data[0] && !users[e].admin && !worked) {
                    ...
                    if (reason !== '') {
                        console.log('[ADMIN] User ' + users[e].name + ' atıldı. Yetkili: ' + currentPlayer.name + ' Bağlantı Sorunu ' + reason);
                    } else {
                        console.log('[ADMIN] User ' + users[e].name + ' atıldı. Yetkili: ' + currentPlayer.name);
                    }
                    socket.emit('serverMSG', 'User ' + users[e].name + ' atıldı. Yetkili: ' + currentPlayer.name);
    ...
    socket.on('1', function () {
        ...
        for (var i = 0; i < currentPlayer.cells.length; i++) {
            if (currentPlayer.cells[i].mass >= c.defaultPlayerMass + c.fireFood && c.fireFood > 0 || currentPlayer.cells[i].mass >= 20 && c.fireFood === 0) {
                var masa = 1;
                if (c.fireFood > 0) masa = c.fireFood;else masa = currentPlayer.cells[i].mass * 0.1;
    ...
    function check(user) {
        for (var i = 0; i < user.cells.length; i++) {
            if (user.cells[i].mass > 10 && user.id !== currentPlayer.id) {
                ...
                var collided = SAT.testCircleCircle(playerCircle, new C(new V(user.cells[i].x, user.cells[i].y), user.cells[i].radius), response);
                if (collided) {
...
function sendUpdates() {
    users.forEach(function (u) {
        ...
        var visibleVirus = virus.map(function (f) {
            if (f.x > u.x - u.screenWidth / 2 - f.radius && f.x < u.x + u.screenWidth / 2 + f.radius && f.y > u.y - u.screenHeight / 2 - f.radius && f.y < u.y + u.screenHeight / 2 + f.radius) {
                return f;
            }
        }).filter(function (f) {
            return f;
        ...
        var visibleMass = massFood.map(function (f) {
            if (f.x + f.radius > u.x - u.screenWidth / 2 - 20 && f.x - f.radius < u.x + u.screenWidth / 2 + 20 && f.y + f.radius > u.y - u.screenHeight / 2 - 20 && f.y - f.radius < u.y + u.screenHeight / 2 + 20) {
                return f;
            }
        }).filter(function (f) {
            return f;
        ...
        var visibleCells = users.map(function (f) {
            for (var z = 0; z < f.cells.length; z++) {
                if (f.cells[z].x + f.cells[z].radius > u.x - u.screenWidth / 2 - 20 && f.cells[z].x - f.cells[z].radius < u.x + u.screenWidth / 2 + 20 && f.cells[z].y + f.cells[z].radius > u.y - u.screenHeight / 2 - 20 && f.cells[z].y - f.cells[z].radius < u.y + u.screenHeight / 2 + 20) {
                    z = f.cells.lenth;
                    if (f.id !== u.id) {
                        return {
                            id: f.id,
